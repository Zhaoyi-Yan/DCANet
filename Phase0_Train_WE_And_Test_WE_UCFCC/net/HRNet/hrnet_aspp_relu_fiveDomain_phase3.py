...
class HighResolutionNet(nn.Module):
    ...
    def __init__(self, leaky_relu=False, attn_weight=1, fix_domain=1, domain_center_model='', **kwargs):
      ...
      #      self.stage4_cfg, num_channels, multi_scale_output=True)
        ...
        G_NWPU = torch.from_numpy(domain_center_src['G_NWPU']).view(1, -1, 1, 1)
        G_JHU = torch.from_numpy(domain_center_src['G_JHU']).view(1, -1, 1, 1)

        self.n_domain = 5

        self.G_all = torch.cat([G_SHA.clone(), G_SHB.clone(), G_QNRF.clone(), G_NWPU.clone(), G_JHU.clone()], dim=0)

        self.G_all = nn.Parameter(self.G_all)

#        self.G_SHA = nn.Parameter(G_SHA)
#        self.G_SHB = nn.Parameter(G_SHB)
#        self.G_QNRF = nn.Parameter(G_QNRF)
    ...
    #    x3 = F.interpolate(x[3], size=(x0_h, x0_w), mode='bilinear', align_corners=False)
       ...
       # x = torch.cat([x[0], x1, x2, x3], 1)
        ...
        if self.cosine == 1:
            G_A, G_B, G_Q, G_N, G_J = torch.chunk(G_all_d, 5, dim=0)
            ...
            cos_dis_N = F.cosine_similarity(pred_attn_d, G_N, dim=1).view(-1)
            cos_dis_J = F.cosine_similarity(pred_attn_d, G_J, dim=1).view(-1)


            cos_dis_all = torch.stack([cos_dis_A, cos_dis_B, cos_dis_Q, cos_dis_N, cos_dis_J]).view(bz, -1) # bz*5
            ...
            target_attn = torch.sum(target_attn, dim=1, keepdim=False) # bz * 512 * 1 * 1
