...
class HighResolutionNet(nn.Module):
    ...
    def __init__(self, leaky_relu=False, attn_weight=1, fix_domain=1, domain_center_model='', **kwargs):
      ...
      #      self.stage4_cfg, num_channels, multi_scale_output=True)
        ...
        G_SHA = torch.from_numpy(domain_center_src['G_SHA'])
        G_SHB = torch.from_numpy(domain_center_src['G_SHB'])
        G_QNRF = torch.from_numpy(domain_center_src['G_QNRF'])
    ...
    # when attn_weights=None, meaning it is in testing phase
    ...
    #    x3 = F.interpolate(x[3], size=(x0_h, x0_w), mode='bilinear', align_corners=False)
       ...
       # x = torch.cat([x[0], x1, x2, x3], 1)
        ...
        bz = x.size(0)
        if torch.is_tensor(domain): # training
            target_all = []
            for i in range(bz):
                if domain[i].item() == 0:
                    target_all.append(self.G_SHA.view(1, -1, 1, 1))
                elif domain[i].item() == 1:
                    target_all.append(self.G_SHB.view(1, -1, 1, 1))
                elif domain[i].item() == 2:
                    target_all.append(self.G_QNRF.view(1, -1, 1, 1))
            target_all = torch.cat(target_all, dim=0)

            if self.fix_domain:
                target_all = target_all.detach()

            self.attn_loss = self.criterion_attn(pred_attn, target_all) * self.attn_weight
        else:
            self.attn_loss = None
