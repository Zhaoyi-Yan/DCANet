...
class DatasetConstructor(data.Dataset):
    ...
    def resize(self, img, dataset_name):
        ...
        if dataset_name == "SHA":
            ...
            large_sz = 1280
            if resize_width >= large_sz:  # use 1024 here to avoid OOM
                tmp = resize_width
                resize_width = large_sz
                resize_height = (resize_width / tmp) * resize_height

            if resize_height >= large_sz:
                tmp = resize_height
                resize_height = large_sz
                resize_width = (resize_height / tmp) * resize_width
            ...
            if resize_height < resize_width:
                if resize_width / resize_height > large_sz/416: # 1024/416=2.46
                    resize_width = large_sz
                    resize_height = 416
            else:
                if resize_height / resize_width > large_sz/416:
                    resize_height = large_sz
                    resize_width = 416
...
class TrainDatasetConstructor(DatasetConstructor):
    ...
    def __getitem__(self, index):
        if self.mode == 'crop':
            ...
            # get domain
            class_id = 0
            if img_path.find('SHA') != -1:
                class_id = 0
            elif img_path.find('SHB') != -1:
                class_id = 1
            elif img_path.find('QNRF') != -1:
                class_id = 2
            elif img_path.find('NWPU') != -1:
                class_id = 3
            elif img_path.find('JHU') != -1:
                class_id = 4
            else:
                assert 1==2
...
class EvalDatasetConstructor(DatasetConstructor):
    ...
    def __getitem__(self, index):
        ...
        return img_path, img_ori_tensor, img, gt_map.view(1, gt_shape[1] // 2, gt_shape[2] // 2), class_id
