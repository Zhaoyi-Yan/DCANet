...
class HighResolutionNet(nn.Module):
    ...
    def forward(self, x, zero_index, weight_scale):
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.conv2(x)
    ...
    #    x3 = F.interpolate(x[3], size=(x0_h, x0_w), mode='bilinear', align_corners=False)
       ...
       # x = torch.cat([x[0], x1, x2, x3], 1)
        ...
        aspp_out_cl = torch.cat(aspp_out, dim=1)

        aspp_out = torch.chunk(aspp_out_cl, 4, dim=1)
  ...
  #      else:
  #          cur_b = zero_index // 128
  #          cur_c = zero_index % 128
  #          mask = torch.ones_like(x)
  #          mask[:, cur_c, :, :] = mask[:, cur_c, :, :] * weight_scale
