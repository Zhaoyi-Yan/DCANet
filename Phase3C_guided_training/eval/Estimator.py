...
class Estimator(object):
    ...
    def evaluate(self, net, is_save=False):
        ...
        imgs_cnt = [0, 0, 0, 0, 0] # logging for each dataset
        pred_mae = [0, 0, 0, 0, 0]
        pred_mse = [0, 0, 0, 0, 0]

        cur, time_cost = 0, 0
        for eval_img_path, eval_ori_img, eval_img, eval_gt, class_id in self.eval_loader:
            ...
            with torch.no_grad():
                eval_prediction, _, _ = net(eval_patchs) # discard class_id
                ...
                if is_save:
                    ...
                    util.save_image(util.tensor2im(eval_ori_img), os.path.join('out_imgs', os.path.splitext(eval_img_path.split('/')[-1])[0]+'.jpg'))
