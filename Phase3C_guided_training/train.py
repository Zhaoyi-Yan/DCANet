...
high_pred_mae = [1e6, 1e6, 1e6, 1e6, 1e6]
...
for epoch_index in range(setting.epoch):
    ...
    for train_img, train_gt, class_id in train_loader:
        ...
        start = time.time()
        prediction, pred_attn, target_attn = net(x, domain)
    ...
    if epoch_index % opt.eval_per_epoch == 0 and epoch_index > opt.start_eval_epoch:
        ...
        log_f.write(
            ...
            .format(step, epoch_index, validate_loss, validate_MAE, validate_RMSE, pred_mae[0], pred_mae[1], pred_mae[2], pred_mae[3], pred_mae[4],
                       pred_mse[0], pred_mse[1], pred_mse[2], pred_mse[3], pred_mse[4], time_cost))
        log_f.flush()
        ...
        if pred_mae[0] < high_pred_mae[0] or pred_mae[1] < high_pred_mae[1] or pred_mae[2] < high_pred_mae[2] or \
           pred_mae[3] < high_pred_mae[3] or pred_mae[4] < high_pred_mae[4]:
            high_pred_mae[0] = pred_mae[0]
            high_pred_mae[1] = pred_mae[1]
            high_pred_mae[2] = pred_mae[2]
            high_pred_mae[3] = pred_mae[3]
            high_pred_mae[4] = pred_mae[4]
            save_now = True
        ...
        if save_now:
            best_model_name = setting.model_save_path + "/MAE_" + str(round(validate_MAE, 2)) + \
                 ...
                 '_' + str(round(pred_mae[1], 2)) + '_' + str(round(pred_mae[2], 2)) + '_' + str(round(pred_mae[3], 2)) + '_' + str(round(pred_mae[4], 2)) + \
                 '_mse_' + str(round(pred_mse[0], 2)) + \
                 '_' + str(round(pred_mse[1], 2)) + '_' + str(round(pred_mse[2], 2)) + '_' + str(round(pred_mse[3], 2)) + '_' + str(round(pred_mse[4], 2)) + \
