...
class DatasetConstructor(data.Dataset):
    ...
    def resize(self, img, dataset_name):
        ...
        resize_width = width
        if dataset_name == "SHA" or dataset_name == 'UCF_CC_50':
    ...
    def __getitem__(self, index):
        if self.mode == 'crop':
            ...
            if os.path.basename(img_path).find('SHA') != -1:
                ...
                class_id = 2
            elif os.path.basename(img_path).find('UCF_CC_50') != -1:
                class_id = 3
            elif os.path.basename(img_path).find('WorldExpo') != -1:
                class_id = 4
            else:
                assert 1==2
