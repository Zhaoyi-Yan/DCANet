...
def select_optim(net, opt):
    if opt.optimizer == 'adam':
         target_key = ['module.G_all']
         target_params = list(map(lambda x:x[1], list(filter(lambda kv: kv[0] in target_key, net.named_parameters()))))
         base_params = list(map(lambda x:x[1], list(filter(lambda kv: kv[0] not in target_key, net.named_parameters()))))
         optimizer = torch.optim.Adam([{'params': target_params, 'lr':opt.target_lr * opt.lr},
                                       {'params': base_params},
                                       ],
                     opt.lr, weight_decay=opt.weight_decay, amsgrad=opt.amsgrad)
...
def define_net(opt):
    ...
    if net_name == 'res_unet':
        ...
        net = HRNet_aspp_relu(attn_weight=opt.loss_attn_weight, fix_domain=opt.fix_domain, domain_center_model=opt.domain_center_model)
        net.init_weights('hrnetv2_w40_imagenet_pretrained.pth')
