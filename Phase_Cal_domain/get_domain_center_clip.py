...
npys = glob.glob("mae56.4_fast_out_oriImg/*npz")
...
for npy in npys:
    ...
    attn_info = attn_source['delta_loss'] # loss is mae here for fast version
    attn_info = torch.from_numpy(attn_info).view(-1, 1)
...
print('SHA: ', str(len(sha_lst)), ' SHB: ', str(len(shb_lst)), ' QNRF: ', str(len(qnrf_lst)))
...
print(dMAE_per_c_SHA)
print(dMAE_per_c_SHB)
...
np.savez('average_clip_domain_center_56.4.npz', G_SHA = G_SHA, G_SHB = G_SHB, G_QNRF = G_QNRF)
