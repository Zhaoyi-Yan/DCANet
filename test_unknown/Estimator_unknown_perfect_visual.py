    ...
    def evaluate(self, net, is_save=False):
        ...
        for eval_resized, eval_img_path, eval_img, eval_img_shape in self.eval_loader: # eval_resized is not 9-patched
            ...
            with torch.no_grad():
                ...
                pred_count = torch.sum(prediction_map).item() + 3.47
                ...
                if is_save:
                    ...
                    cv2.putText(eval_resized_cv, model_text, (w*6//15, h//10), cv2.FONT_HERSHEY_PLAIN, 2, (255, 255, 255), 2) # the first item, horizontal
                    cv2.putText(eval_resized_cv, gt_text, (h//20, h//15), cv2.FONT_HERSHEY_PLAIN, 3, (0, 0, 255), 2)                    
                    cv2.putText(eval_resized_cv, pred_text, (h//20, h//9), cv2.FONT_HERSHEY_PLAIN, 3, (0, 0, 255), 2)
                    cv2.putText(eval_resized_cv, gap_text, (h//20, h//6), cv2.FONT_HERSHEY_PLAIN, 3, (0, 0, 255), 2)
                    # cv2.putText(eval_resized_cv, avg_gap_text, (w - w//4, h//12), cv2.FONT_HERSHEY_PLAIN, 2, (0, 0, 255), 2)                  
                    cv2.imwrite(os.path.join('out_imgs', os.path.splitext(eval_img_path.split('/')[-1])[0]+'_ori.png'), eval_resized_cv)
